...
module Highcharts_Map_Chart
{
    type Series = [string, number][];

    export function draw(chart : Map_Chart, structure : Structure)
    {
    ...
    function convert_data_to_series(chart : Map_Chart, data : Data)
	{
    ...
    function compute_colors_classes(series : Series, chart : Map_Chart)
    {
        let values = series.map(x => x[1]);

        let colors = Utils.map_values_to_colors(
            values, 
            chart.get_min_color(), 
            chart.get_middle_color(), 
            chart.get_max_color(), 
            chart.get_middle_value()
        );
    ...
    function get_options(chart : Map_Chart, structure : Structure)
	{
		return {
			title:         { text: "" },
			subtitle:      { text: "" },
    ...
    function get_chart_options(chart : Map_Chart, structure : Structure)
    {
        let map_area = chart.get_map_area();
        let map_area_code = "custom/europe";
        if (map_area === "FR-DEP") map_area_code = "countries/fr/fr-all-all";
        if (map_area === "FR-REG") map_area_code = "countries/fr/fr-all";
    ...
    function get_series_options(chart : Map_Chart)
    {
        return [{
            data: {},
            name: "Name",
    ...
    function get_tooltip_options(chart : Map_Chart)
    {
        return {
    ...
    function get_mapping(map_area : Map_Area)
    {
        if (map_area === "UE")     return get_EU_mapping();
        if (map_area === "FR-DEP") return get_FR_DEP_mapping();
        if (map_area === "FR-REG") return get_FR_REG_mapping();

        return {};
    }

    function get_EU_mapping() : { [area : string] : string}
    {
        return {
            "austria": "at",
            "autriche": "at",
    ...
    function get_FR_REG_mapping() : { [area : string] : string}
    {
        return {
            "11": "fr-idf",
            "24": "fr-cvl",
    ...
    function get_FR_DEP_mapping() : { [area : string] : string}
    {
        return {
            "1": "fr-ara-ai",
            "2": "fr-hdf-as",
